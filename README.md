...
2. **On the slave Jetson** run the ROS 2 CAN-FD router so that the MCU switches to `FORCE_DIRECT` mode (same as in `titati_control`).
    ...
    cd rl_sar
    source /opt/ros/humble/setup.bash
    source install/local_setup.bash    # generated by ./build.sh tita_system_interfaces titati_canfd_router
    # optional sanity check before launching the node
    ros2 pkg executables titati_canfd_router
