...
1. **在两台 Jetson 上各执行一次硬件构建。** `-m` 预设现在会自动检测并加载系统中的 ROS（若未手动 `source` 也没关系），随后一次性编译真实控制器、电机测试工具、电池守护进程以及 CAN 心跳程序。无需再单独构建 `battery_device`、`hardware_bridge` 或 `hw_bringup`。
...
```
   ...
   | `battery_device_node` | 发布电池诊断并提供 `/power_state_set`、`/power_heart_beat`、`/power_self_test` 服务。 |
   | `titati_canfd_router_node` | 透传 CAN 心跳，保持电源板在线。 |

2. **先在从机 Jetson 上执行辅助脚本。** 脚本会配置 `can0`、停止旧的 `tita-bringup` 服务，并在后台启动 ROS 电源守护进程，日志保存在 `cmake_build/logs/*_slave.log`。
...
```
   ...
   从机会在命名空间 `tita`（或你设置的 `TITATI_NAMESPACE`）下启动 `battery_device_node` 和 `titati_canfd_router_node`，维持电池供电并应答电源心跳，使主机能够接管控制。

3. **再在主机 Jetson 上执行同样的脚本。** 它会进行相同的 CAN 配置、重启电源相关服务，并为强化学习程序准备环境。日志同样位于 `cmake_build/logs/`。
...
```
   ...
   两台 Jetson 完成脚本后，将分别在线运行下列 ROS 节点：

   | 节点 | 命名空间 | 接口 |
   | --- | --- | --- |
   | `battery_device_node` | `/tita` | 服务 `/power_state_set`、`/power_heart_beat`、`/power_self_test`；话题 `/left_battery_state`、`/right_battery_state`、`/power_domain_info_diagnostic` 等 |
   | `titati_canfd_router_node` | `/tita` | 转发 CAN FD 数据并维持电源板心跳 |

   若遇到重启或 CAN 掉线，可随时重新运行脚本；它们会先结束旧进程再重新拉起。

4. **将策略文件拷贝到** `src/rl_sar/policy/titati/robot_lab/`，并按需修改 `config.yaml`。
...
6. **从机准备就绪后，在主机 Jetson 上启动强化学习控制程序：**
...
```
   ...
   键盘按键与前文表格一致（WASD 控制移动，Q/E 控制偏航，空格停止，`N` 切换导航模式）。无需再启动 `titati_control` 中的任何包，一切均已迁移到 `rl_sar`。
