...
- On the slave Jetson, after the MCU finishes its self-check, run the CAN router bundled in this package so every frame is forwarded transparently. The launcher auto-detects whether ROS is available:

  ```bash
  # Standalone hardware deployment (./build.sh -m or auto-fallback)
  ./src/rl_sar/scripts/run_titati_canfd_router.sh

  # If you built inside a ROS workspace
  source install/setup.bash        # ROS 2
  # or: source devel/setup.bash    # ROS 1
  rl_sar/src/rl_sar/scripts/run_titati_canfd_router.sh
  ```

  If ROS commands are not available or the package is not installed, the script automatically falls back to `cmake_build/bin/titati_canfd_router_node`. The node keeps listening for the control-board heartbeat and automatically issues the `SET_READY_NEXT -> FORCE_DIRECT` CAN RPC when needed. When the router is in pass-through mode the master Jetson must broadcast the standard direct-mode RPC frame on the shared CAN-FD bus; the bundled Titati SDK matches the original `titati_control` behaviour so both motor controllers see the command on the default broadcast ID.
...
- `--log-interval <sec>`: how often to print the measured position/velocity/torque readbacks.
  ...
  The diagnostic repeatedly requests forced-direct mode, waits for MCU telemetry, and prints live readbacks while the command is active. Before sending any torque it prints which joints are already streaming feedback (timestamped packets) so you can confirm that all **16** actuators are online. If the tool reports that an entire half of the joints (either `0-7` or `8-15`) is missing, the corresponding motor controller has not yet switched to direct mode; double-check the CAN-FD router and MCU state for that side. In both cases the test automatically replays the direct-mode RPC so you can immediately retry once the router or MCU is ready.
