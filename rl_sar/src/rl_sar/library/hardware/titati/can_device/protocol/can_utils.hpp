...
public:
  ...
  {
    canid_t send_id = 0;
    send_id = (send_id | (static_cast<canid_t>(can_id_offset_) << 8));
    send_id = (send_id | tx_frame.can_id);
    tx_frame.can_id = send_id;
    return tx_op_ != nullptr && tx_op_->send_can_message(tx_frame);
  }
  ...
  std::shared_ptr<canfd_frame> wait_for_can_data_block()
  {
    return rx_op_ != nullptr ? rx_op_->wait_for_can_data_block() : nullptr;
  }
