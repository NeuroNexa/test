...
#define CAN_MASK_AS_MOTORS_INFO (0x7E0U)
    std::string motors_can_interface = detail::get_env_or_default("TITATI_CAN_RX_INTERFACE", detail::get_env_or_default("TITATI_CAN_INTERFACE", "can0"));
    std::string motors_can_name = detail::get_env_or_default("TITATI_CAN_RX_NAME", "motors_can");
    bool motors_can_extended_frame = false;
    bool motors_can_rx_is_block = false;
    int64_t motors_timeout_us = MAX_TIME_OUT_US;
    uint8_t motors_can_id_offset = detail::get_env_u8("TITATI_CAN_RX_ID_OFFSET", detail::get_env_u8("TITATI_CAN_ID_OFFSET", 0x00U));
