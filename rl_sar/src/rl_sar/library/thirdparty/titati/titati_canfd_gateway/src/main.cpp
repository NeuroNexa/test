...
#include "titati_canfd_gateway/canfd_router_can_receive_api.hpp"
...
public:
  explicit CanfdRouterCanNode(const rclcpp::NodeOptions & option) : Node("titati_slave_controller_node", option) {
    ...
    power_self_test_client_ = this->create_client<titati_system_interfaces::srv::PowerSelfTestSrv>(
    tita_topic::kpower_self_test_service, client_qos);
    auto request = std::make_shared<titati_system_interfaces::srv::PowerSelfTestSrv::Request>();
...
private:
  ...
  rclcpp::Client<titati_system_interfaces::srv::PowerSelfTestSrv>::SharedPtr power_self_test_client_ =
    nullptr;
  void service_response_callback(rclcpp::Client<titati_system_interfaces::srv::PowerSelfTestSrv>::SharedFuture response) {
      (void)response;
