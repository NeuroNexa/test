...
void RLTensorRT_Handle::Forward()
{
    GetObs();
...
void RLTensorRT_Handle::Run_Forward()
{
    while(threadRunning)
    {
...
void RLTensorRT_Handle::Package_Init(LegData* legdata, AttitudeData* attidata)
{
   ...
   //    obs_buf = torch::zeros({history_length,30},torch::kCUDA);
    ...
    threadRunning = true;
    if(thread_first_) {
      forward_thread = std::thread(&RLTensorRT_Handle::Run_Forward,this);
      thread_first_ = false;
    }
}

void RLTensorRT_Handle::Package_Run()
{   
    for(int i = 0; i < 4; i++)
    {
