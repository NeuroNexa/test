...
void RLTensorRT_SRR_Handle::Forward()
{
    GetObs();
...
void RLTensorRT_SRR_Handle::GetObs()
{
...
void RLTensorRT_SRR_Handle::Run_Forward()
{
    while(threadRunning)
    {
...
void RLTensorRT_SRR_Handle::Package_Init(LegData* legdata, AttitudeData* attidata)
{
    ...
    //                               (double)legdata->q_abad[2] + 1.57, -(double)legdata->q_hip[2], -(double)legdata->q_knee[2]}});
        ...
        const float default_dof_pos_tmp[16] = {-0.1, 0.8, -1.5, 0, 0.1, 0.8, -1.5, 0, -0.1, -1.0, 1.5, 0, 0.1, -1.0, 1.5, 0};
        for(int i = 0; i < 16; i++) {
   ...
   //    obs_buf = torch::zeros({history_length,30},torch::kCUDA);
    ...
    threadRunning = true;
    if(thread_first_) {
      forward_thread = std::thread(&RLTensorRT_SRR_Handle::Run_Forward,this);
      thread_first_ = false;
    }
}

void RLTensorRT_SRR_Handle::Package_Run()
{   
    for(int i = 0; i < 4; i++)
    {
